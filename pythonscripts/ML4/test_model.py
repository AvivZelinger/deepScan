...
##########################################
# The table now includes columns: name, size, type, size_field
...
def parse_pcap(pcap_file, protocol_df):
    ...
    For each field:
      - If 'size' is non-zero, that value is used.
      - If 'size' is 0 (dynamic field), the 'size_field' is checked:
          - If provided, its value (from packet data) is used.
          - Otherwise, the remaining payload length is used.
    
    Returns a list of records (dicts) with:
...
def preprocess_field_data(field_records, preprocessor):
    """
    Converts and processes field data (list of dicts with keys:
    'size', 'value', and 'field_type') and returns a processed feature matrix.
    
    It computes two new features:
      - value_numeric: numeric conversion for 'int'/'float' fields (0.0 otherwise)
      - text_length: length of the text for non-numeric fields (0 for numeric)
    
    The preprocessor was fitted on the features ['size', 'value_numeric', 'text_length'].
    """
...
def generate_dpi(endpoints, model, preprocessor, label_encoder):
    """
    Generates DPI for each endpoint.
    Returns a dictionary with DPI for each IP address.
    """
    dpi_result = {}

    for endpoint_ip, packets in endpoints.items():
        field_stats = defaultdict(list)
        # Collect all records for each endpoint
        ...
        for field_name, stats in field_stats.items():
            ...
            # Determine min_value and max_value based on the field type
            ...
            # Check for a size-defining field (if any)
            ...
            # Build field data including the field type so that preprocessing matches training
            field_data = [{
                'size': s['size'],
                'value': s['value'],
            ...
            if X is None:
                ...
                # Get average confidence as accuracy metric
                accuracy = float(np.mean(confidences))
                print(f"Field: {field_name}, Accuracy: {accuracy}")
                # Use the mode of the predictions as the field type
...
def parse_pcap_with_ip(pcap_file, protocol_df):
    """
    Similar to parse_pcap, but returns a dict organized by IP address.
    """
    packets = rdpcap(pcap_file)

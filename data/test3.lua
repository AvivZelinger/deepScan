-- Wireshark Lua static dissector for test3
-- Decodes fields by fixed sizes (no DPI tests), showing field summary in Info.

local test3 = Proto("test3", "test3")

local f_signature = ProtoField.string("test3.signature", "Signature")
local f_version = ProtoField.uint8("test3.version", "Version"), base.DEC
local f_flags = ProtoField.uint8("test3.flags", "Flags"), base.DEC
local f_command = ProtoField.string("test3.command", "Command")
local f_session_id = ProtoField.uint32("test3.session_id", "Session_id"), base.DEC
local f_msg_id = ProtoField.uint32("test3.msg_id", "Msg_id"), base.DEC
local f_timestamp = ProtoField.uint64("test3.timestamp", "Timestamp"), base.DEC
local f_payload_size = ProtoField.uint32("test3.payload_size", "Payload_size"), base.DEC
local f_message = ProtoField.string("test3.message", "Message")

test3.fields = { f_signature, f_version, f_flags, f_command, f_session_id, f_msg_id, f_timestamp, f_payload_size, f_message }

function test3.dissector(buffer, pinfo, tree)
    if buffer:len() == 0 then return end
    pinfo.cols.protocol = "test3"
    local subtree = tree:add(test3, buffer(), "test3")
    local offset = 0
    local field_values = {}

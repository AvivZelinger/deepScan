-- Wireshark Lua static dissector for newTesttttt
-- Decodes fields by fixed sizes (no DPI tests), showing field summary in Info.

local newTesttttt = Proto("newTesttttt", "newTesttttt")

local f_signature = ProtoField.string("newTesttttt.signature", "Signature")
local f_version = ProtoField.uint8("newTesttttt.version", "Version"), base.DEC
local f_flags = ProtoField.uint8("newTesttttt.flags", "Flags"), base.DEC
local f_flags_bf0 = ProtoField.uint8("newTesttttt.flags_bf0", "Flags Bitfield 1", base.DEC)
local bf_fields_flags = { f_flags_bf0 }
local f_command = ProtoField.string("newTesttttt.command", "Command")
local f_session_id = ProtoField.uint32("newTesttttt.session_id", "Session_id"), base.DEC
local f_msg_id = ProtoField.uint32("newTesttttt.msg_id", "Msg_id"), base.DEC
local f_timestamp = ProtoField.uint64("newTesttttt.timestamp", "Timestamp"), base.DEC
local f_payload_size = ProtoField.uint32("newTesttttt.payload_size", "Payload_size"), base.DEC
local f_message = ProtoField.string("newTesttttt.message", "Message")

newTesttttt.fields = { f_signature, f_version, f_flags, f_flags_bf0, f_command, f_session_id, f_msg_id, f_timestamp, f_payload_size, f_message }

function newTesttttt.dissector(buffer, pinfo, tree)
    if buffer:len() == 0 then return end
    pinfo.cols.protocol = "newTesttttt"
    local subtree = tree:add(newTesttttt, buffer(), "newTesttttt")
    local offset = 0
    local field_values = {}
...
udp_port:add(10000, newTesttttt)

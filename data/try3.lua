-- Wireshark Lua static dissector for try3
-- Decodes fields by fixed sizes (no DPI tests), showing field summary in Info.

local try3 = Proto("try3", "try3")

local f_signature = ProtoField.string("try3.signature", "Signature")
local f_version = ProtoField.uint8("try3.version", "Version"), base.DEC
local f_flags = ProtoField.uint8("try3.flags", "Flags"), base.DEC
local f_flags_bf0 = ProtoField.uint8("try3.flags_bf0", "Flags Bitfield 1", base.DEC)
local bf_fields_flags = { f_flags_bf0 }
local f_command = ProtoField.string("try3.command", "Command")
local f_session_id = ProtoField.uint32("try3.session_id", "Session_id"), base.DEC
local f_msg_id = ProtoField.uint32("try3.msg_id", "Msg_id"), base.DEC
local f_timestamp = ProtoField.uint64("try3.timestamp", "Timestamp"), base.DEC
local f_payload_size = ProtoField.uint32("try3.payload_size", "Payload_size"), base.DEC
local f_message = ProtoField.string("try3.message", "Message")

try3.fields = { f_signature, f_version, f_flags, f_flags_bf0, f_command, f_session_id, f_msg_id, f_timestamp, f_payload_size, f_message }

function try3.dissector(buffer, pinfo, tree)
    if buffer:len() == 0 then return end
    pinfo.cols.protocol = "try3"
    local subtree = tree:add(try3, buffer(), "try3")
    local offset = 0
    local field_values = {}

-- Wireshark Lua static dissector for feb_25
-- Decodes fields by fixed sizes (no DPI tests), showing field summary in Info.

local feb_25 = Proto("feb_25", "feb_25")

local f_proto_id = ProtoField.string("feb_25.proto_id", "Proto_id")
local f_version = ProtoField.string("feb_25.version", "Version")
local f_msg_type = ProtoField.string("feb_25.msg_type", "Msg_type")
local f_session_id = ProtoField.uint32("feb_25.session_id", "Session_id"), base.DEC
local f_seq_num = ProtoField.uint32("feb_25.seq_num", "Seq_num"), base.DEC
local f_timestamp = ProtoField.string("feb_25.timestamp", "Timestamp")
local f_payload_length = ProtoField.uint32("feb_25.payload_length", "Payload_length"), base.DEC
local f_message_data = ProtoField.string("feb_25.message_data", "Message_data")

feb_25.fields = { f_proto_id, f_version, f_msg_type, f_session_id, f_seq_num, f_timestamp, f_payload_length, f_message_data }

function feb_25.dissector(buffer, pinfo, tree)
    if buffer:len() == 0 then return end
    pinfo.cols.protocol = "feb_25"
    local subtree = tree:add(feb_25, buffer(), "feb_25")
    local offset = 0
    local field_values = {}

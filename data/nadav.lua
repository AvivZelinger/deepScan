-- Wireshark Lua static dissector for nadav
-- This dissector decodes fields according to fixed sizes without DPI tests.

local nadav = Proto("nadav", "nadav")

local f_sync = ProtoField.uint8("nadav.sync", "Sync"), base.DEC
local f_id = ProtoField.uint32("nadav.id", "Id"), base.DEC
local f_type = ProtoField.uint32("nadav.type", "Type"), base.DEC
local f_length = ProtoField.uint32("nadav.length", "Length"), base.DEC
local f_payload = ProtoField.string("nadav.payload", "Payload")
local f_crc = ProtoField.uint32("nadav.crc", "Crc"), base.DEC
local f_flag = ProtoField.uint8("nadav.flag", "Flag"), base.DEC

nadav.fields = { f_sync, f_id, f_type, f_length, f_payload, f_crc, f_flag }

function nadav.dissector(buffer, pinfo, tree)
    if buffer:len() == 0 then return end
    pinfo.cols.protocol = "nadav"
    local subtree = tree:add(nadav, buffer(), "nadav")
    local offset = 0
